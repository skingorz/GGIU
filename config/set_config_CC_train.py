...
import time 
import os
method, task, taskid, wandb_project = sys.argv[1:]
...
def config():
    ...
    if config_dict["load_pretrained"]:
        config_dict["pre_trained_path"] = "results/main/protoNet/5shot/FSLdoing/PN_codebase/checkpoints/epoch=53-step=26999.ckpt"
    ...
    trainer["logger"] = {"class_path":"pytorch_lightning.loggers.WandbLogger",
                        "init_args": {"save_dir": os.path.join(log_dir, exp_name), "log_model": True, 
                            "project": wandb_project,"name": f"{branch}/{method}/{task}", "id": taskid}}
